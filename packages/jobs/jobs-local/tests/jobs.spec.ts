...
describe('LocalJobRegistry owner isolation', () => {
  it('fences every view operation to the owning session and keeps unowned jobs out of sessions', async () => {
    const ctx = await harness()
    const owner = await liveAgent(ctx, 'owner')
    ...
    expect(ctx.jobs.read(open).job.id).toBe(open)
    expect(() => ctx.jobs.read(open, other.id)).toThrow('belongs to another session')
  ...
  it('list() shows a session only its OWN jobs — an unowned one is not open to it', async () => {
    ...
    // person that was harmless; hosted, it means alice sees a neighbour's
    // commands and can read their output. Owned ↔ owner, unowned ↔ no session.
    expect(ctx.jobs.list(alice.id).map(t => t.id)).toEqual([aliceTask])
    expect(ctx.jobs.list(bob.id).map(t => t.id)).toEqual([bobTask])
  ...
  it('a session cannot read, kill or wait on an unowned job', async () => {
    ...
    expect(() => ctx.jobs.read(openTask, alice.id)).toThrow('belongs to another session')
    expect(() => ctx.jobs.kill(openTask, alice.id)).toThrow('belongs to another session')
    expect(() => ctx.jobs.get(openTask, alice.id)).toThrow('belongs to another session')
  })
