...
export class LocalJobRegistry extends JobRegistry {
  ...
  /**
   * The isolation fence, symmetric: an owned job is reachable only by its
   * owner, and an unowned job only by a caller that is likewise not a session.
   *
   * An unowned job used to be open — listable and killable from every session
   * in the process. That is harmless when one process serves one person, and
   * is a cross-session leak the moment it does not: `job_list` shows another
   * session's commands and `job_output` hands over their stdout. Nothing
   * in-tree relies on the old reach; every caller that starts a job passes an
   * owner except bash/pwsh with no agent context, and such a job has no
   * session that should see it either.
   */
  private mayAccess(job: TrackedJob, caller: SessionId | undefined): boolean {
    return job.owner === undefined ? caller === undefined : job.owner.id === caller
  }
